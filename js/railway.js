...
async function getRailway(data) {
  ...
  if (data["start"]!=data["terminal"]) {
    ...
    let time = await getTimeApiResponse(
      distanceUrl,
      direction,
      todayString,
      accesstoken
    ...
    const railway_short_drive_time = document.querySelectorAll(
      "#railway_short_drive_time"
    )[0];
    const railway_long_drive_time = document.querySelectorAll(
      "#railway_long_drive_time"
    )[0];
    const railway_short_drive_price = document.querySelectorAll(
      "#railway_short_drive_price"
    )[0];
    const railway_long_drive_price = document.querySelectorAll(
      "#railway_long_drive_price"
    )[0];
...
async function getTimeApiResponse(
  priceUrl,
  direction,
  todayString,
  accesstoken
  ...
  const res = await fetch(priceUrl, requestOptions)
    ...
    .then((response) => {
      ...
      response["TrainTimetables"].forEach((element) => {
        ...
        let [departureTimeHours, departureTimeMinutes] =
          departureTime.split(":");
      ...
      let lowerest_minute = Math.ceil(
        (lowerest_time / 3600 - lowerest_hr) * 60
      );
      let highest_hr = Math.floor(highest_time / 3600);
      let highest_minute = Math.ceil((highest_time / 3600 - highest_hr) * 60);

      return {
        lowerest_time: `${lowerest_hr}小時${lowerest_minute}分`,
        highest_time: `${highest_hr}小時${highest_minute}分`,
      };
    });
  return res;
}
